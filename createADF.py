...
def avg_medal_count():
    ...
    # YOUR CODE HERE
    bronzec = numpy.mean(df['bronze'])
    silverc = numpy.mean(df['silver'])
    goldc = numpy.mean(df['gold'])
    avg_medal_count = Series([goldc, silverc, bronzec], index=['gold', 'silver', 'bronze'])
    avg_medal_count = df['gold', 'silver', 'bronze'].apply(numpy.mean)
    vals = [4, 2, 1]
    medals = [gold, silver, bronze]
    points = numpy.dot(vals, medals)
